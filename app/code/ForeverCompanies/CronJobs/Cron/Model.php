   ...
   //     Mage::app()->setCurrentStore($storeId);
    ...
    function getProductsReviews($type) {
        ...
        foreach ($reviews as $review) {
            ...
            $url =  $this->storeManager->getStore()->getBaseUrl() . $product->getUrlKey();
            $content = preg_replace("/(\. \. \.)/",'',preg_replace("/(\.)\\1+/",'',str_replace('&', 'and', strip_tags(html_entity_decode(check_format($review['detail']))))));
            $title = preg_replace("/(\. \. \.)/",'',preg_replace("/(\.)\\1+/",'',str_replace('&', 'and', strip_tags(html_entity_decode(check_format($review['title']))))));
            $name = preg_replace("/(\. \. \.)/",'',preg_replace("/(\.)\\1+/",'',str_replace('&', 'and', strip_tags(html_entity_decode(check_format($review['nickname']))))));
