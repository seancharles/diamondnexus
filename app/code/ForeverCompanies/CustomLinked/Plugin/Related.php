<?php
namespace ForeverCompanies\CustomLinked\Plugin;
...
class Related extends RelatedParent {
    ...
    protected function getCustomlinkedFieldset($modify) {
        $content = __(
                'Matching Bands / Enhancer Products'
        );
        return [
            'children' => [
                'button_set' => $modify->getButtonSet(
                        $content, __('Add Matching Bands / Enhancers'), $modify->scopePrefix . static::DATA_SCOPE_CUSTOMLINKED
                ),
                'modal' => $this->getGenericModal(
                        __('Add Matching Bands / Enhancers'), $modify->scopePrefix . static::DATA_SCOPE_CUSTOMLINKED
                ),
            ...
            'arguments' => [
                'data' => [
                    'config' => [
                        ...
                        'label' => __('Matching Bands / Enhancers'),
                        'collapsible' => false,
    ...
    public function beforeGetLinkedProducts($provider, $product) {
        ...
        $this->product = $objectManager->create('ForeverCompanies\CustomLinked\Model\Product');
