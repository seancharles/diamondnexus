...
class Report extends Action
{
    ...
    {
        $log = $this->logFactory->create()->getCollection();
        $log->addFieldToFilter('datetime', ['gteg' => date('Y-m-d', strtotime($startDate))])
            ->addFieldToFilter('datetime', ['lteg' => date('Y-m-d', strtotime($endDate))])
            ->getSelect()
        ...
        foreach ($logs as &$log) {
            foreach ($failedLogs as $failedLog) {
                if (isset($log['salesforce_table']) &&
                    ...
                    $log['salesforce_table'] == $failedLog['salesforce_table']) {
