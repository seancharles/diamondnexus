...
class AuthorizationRequest extends AbstractDataBuilder
{
    ...
    public function build(array $buildSubject)
    {
        ...
        $merchantId = $this->ecomSystem->getMerchantId();

        $payment = $buildSubject['payment'];
        $paymentAddlInfo = $payment->getPayment()->getAdditionalInformation();
        $leaseId = $paymentAddlInfo['lease_id'];

        return [
            'body' => '',
            'path' => "{$sessionId}/application/confirmation?leaseId={$leaseId}&merchantId={$merchantId}",
            'method' => self::GET
        ];
    }
}
