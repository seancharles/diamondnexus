#!/bin/bash
curl https://$username:$pass@api.github.com/repos/ForeverCompanies/magento2/pulls\?base\="develop" | grep labels/develop
rtn=$?
    if [ $rtn = 0 ]; then
        ls
        pwd
        ...
        ls
        mkdir ../temp 
        mv buildspec.yml devspec.yml script.sh ../temp
