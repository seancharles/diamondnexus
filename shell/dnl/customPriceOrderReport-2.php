<?php
	...
	#$orders->addAttributeToFilter('created_at', array('gt' => date('Y-m-d H:i:s', strtotime('- 444 days'))));
	...
	foreach ($shipped_date as $dd) {	
		$order = Mage::getModel('sales/order')->load($dd->parent_id);
		switch ($order->status) {
			case 'complete':
			case 'Shipped':
			case 'delivered':
				...
				if( $item->getOriginalPrice() != $item->getPrice() && $item->getOriginalPrice() > 0 ) {
					print_r($order->increment_id." ".$item->getOriginalPrice()." != ".$item->getPrice()." && ".$item->getOriginalPrice()." > 0 \n");
					...
					$temp['created_at_ship'] = $dd->created_at;
...
<?php
	...
	#mail('bill.tait@forevercompanies.com','Custom Price Report',$content,'Content-type: text/html');
	#mail('pasekei@gmail.com','Custom Price Report',$content,'Content-type: text/html');
