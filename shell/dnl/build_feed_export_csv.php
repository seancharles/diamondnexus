<?php
	...
	function displayProd($productModel) {
		...
		$title = substr(preg_replace('~[[:cntrl:]]~','',preg_replace('/(\s)+/',' ',preg_replace('/s$/','',$title))),0,70);
		...
		switch($product->getTypeId()) {
			case 'configurable':
				...
				foreach ($childProducts as $simpleModel) {
					...
					if($_product->getStatus() == 1) {
						...
						$simpleTitle = substr(preg_replace('~[[:cntrl:]]~','',preg_replace('/(\s)+/',' ',preg_replace('/s$/','',preg_replace('/ None/','',$title." ".$simpleTitleMods)))),0,70);
			...
			case 'simple':
				...
				$simpleTitleMods = $SGemstone." ".$SCut." ".$SColor." ".$SMetal;
				$title = substr(preg_replace('~[[:cntrl:]]~','',preg_replace('/(\s)+/',' ',preg_replace('/s$/','',preg_replace('/ None/','',$title." ".$simpleTitleMods)))),0,70);
				break;
			default:
				break;
		}
