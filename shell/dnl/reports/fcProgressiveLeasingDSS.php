...
//    require_once $_SERVER['HOME'].'magento//Mage.php';
    ...
    $filename = $_SERVER['HOME'].'/html/var/report/progressive-leasing-dss-' . $date . '.csv';
    ...
    $orderQuery = "SELECT
						...
						o.status,
						o.created_at,
                        MIN(h.created_at) processing_date
					FROM
						sales_flat_order o
					INNER JOIN
                    ...
                    AND
                        h.status IN('processing')
                    AND
                        o.status IN('processing')
					GROUP BY
						o.entity_id
                    HAVING
                       processing_date BETWEEN '" . $fromDate . "' AND '" . $toDate . "';";
    ...
    $mail = new Zend_Mail();
    $mail->setBodyHtml(
            "Daily Progressive Leasing Invoice Report - " . $date .
            "<br />" . 
            "<br />" . 
			$bodyHTML .
            "<br />" .
        ...
        ->setSubject('Daily Progressive Leasing Invoice Report - ' . $date . ( (count($orderResult) == 0) ? ': No Orders Found' : '' ));
    ...
    $attachment->filename = 'progressive-leasing-invoice-' . $date . '.csv';
